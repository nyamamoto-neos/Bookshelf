...
local path = system.pathForFile( _G.appName.."kwkVars.json",  _K.DocumentsDir  )
...
--
local kwkVar = json.decode( jsonFile(_G.appName.."kwkVars.json") )
-- Check for saved variables
function _Class:kwkVarCheck(variable)
	print(_G.appName)
   kwkVar = json.decode( jsonFile(_G.appName.."kwkVars.json") )
   local found = nil
   if kwkVar ~= nil then
      for i = 1, #kwkVar do
